...
class SecureFedAvg(fl.server.strategy.FedAvg):
    def aggregate_fit(self, rnd, results, failures):
        ...
        # Safety: if no results, nothing to do
        if aggregated is None:
            print(f"[SecureFedAvg] No aggregated parameters in round {rnd}, "
                  f"results={len(results)}, failures={len(failures)}")
            return None

        parameters, _ = aggregated
        if parameters is None:
            print(f"[SecureFedAvg] parameters is None in round {rnd}, skipping.")
            return aggregated
